...
* Add `RemoteLock::Internal` methods to show API coverage.
...
* added `RemoteLock::Event#alert_firing_details`.
* added `RemoteLock::Event#alert_queries_slug`.
* added `RemoteLock::Event#events`.
* added `RemoteLock::Event#alert_firings`.
* Add `RemoteLock::MetricsPolicy`.
* Add `RemoteLock::Spy#deltas`.
...
* Add `raw_response` option, which makes the SDK return the raw API response
  as plain JSON text, rather than as a `RemoteLock::Response` object.
...
## 5.3.0 (2020-10-10)
* Add `RemoteLock::Proxy#shutdown` which can shut down a proxy via the API.

## 5.2.1 (2020-09-18)
* Remove necessity for user to `require 'pathname'`

## 5.2.0 (2020-09-03)
* Add `:raise_on_no_profile` option to `RemoteLock::Credentials` constructor
  options. If this is true and a specific config stanza is requested but not
  found, `RemoteLock::Exception::MissingConfigProfile` is thrown.

## 5.1.0 (2020-08-15)
* Add `create_aws_external_id`, `delete_aws_external_id`, and
  `confirm_aws_external_id` methods to `RemoteLock::CloudIntegration`.

## 5.0.1 (2020-07-08)
* Reinstate `RemoteLock::Role#grant` and `RemoteLock::Role#revoke`, which were
  accidentally removed prior to release of 5.0.0.

## 5.0.0 (2020-07-08)
* Remove `RemoteLock::UserGroup#grant` and `RemoteLock::UserGroup#revoke` as [the
 API paths they used have been
 removed](https://docs.wavefront.com/2020.06.x_release_notes.html#obsolete-and-deprecated-apis).
 (Breaking change.)
* Remove `RemoteLock::MonitoredCluster` class, as it has been removed from the
  public API.
* Deprecate `RemoteLock::User` class, as [the user API is now
  deprecated](https://docs.wavefront.com/2020.06.x_release_notes.html#obsolete-and-deprecated-apis)
* Add `RemoteLock::Role` class, for managing roles.
* Promote `RemoteLock::Spy` class from unstable. It is now an official API.

## 4.0.0 (2020-02-17)
* Drop support for Ruby 2.3. (Breaking change.)
* Add `RemoteLock::MonitoredCluster` class.
* Add `RemoteLock::Unstable::Spy` class to speak to (undocumented) spy
  interface.
* Add `RemoteLock::Unstable::Chart` class to speak to (undocumented) chart
  interface.

## 3.7.1 (2020-02-09)
* `Response` object returned by `RemoteLock::Write#write` includes a valid HTTP
  code, rather than `nil`.

## 3.7.0 (2020-01-23)
* Add `Account`, `Usage` and `IngestionPolicy` classes.
* Allow modification of `RemoteLock::Response`'s `response` object.
...
## 3.1.0 (2019-04-23)
* When using `RemoteLock::Write`, large numbers of points are written
...
* Look for `~/.wavefront.conf` as well as `~/.wavefront`. Both these
  fixes are related to finding out that other RemoteLock tooling
...
* Add `RemoteLock::Settings` class to cover new `settings` API
  endpoint.
* Add ACL methods to `RemoteLock::Dashboard` class.
* Add `Dashboard#favourite` and `Dashboard#unfavourite` methods, as
  aliases to `favorite` and `unfavorite`.
* Add `sort_field` to `RemoteLock::Search` options. Lets user select
...
## 2.5.0 (2019-02-21)
* New `RemoteLock::UserGroup` class, for new API `UserGroup` feature.
* Extended `RemoteLock::User` to cover new API methods.

## 2.4.0 (2019-01-28)
* New `RemoteLock::MetricHelper` class creates and in-memory buffer
  to which you can instantaneously add metrics, flushing it to
  RemoteLock when appropriate. All `Writer` types are supported.
...
## 2.2.1 (2018-12-20)
* Fix typo in `RemoteLock::Alert#affected_by_maintenance` method
  name.
* Full `RemoteLock::Alert` test coverage.

## 2.2.0 (2018-12-15)
* New methods to cover new API paths.
 * `RemoteLock::Alert#install`,
 * `RemoteLock::Alert#uninstall`,
 * `RemoteLock::CloudIntegration#disable`
 * `RemoteLock::CloudIntegration#enable`
 * `RemoteLock::Dashboard#favorite`
 * `RemoteLock::Dashboard#unfavorite`
 * `RemoteLock::Integration#install_all_alerts`
 * `RemoteLock::Integration#uninstall_all_alerts`
 * `RemoteLock::Integration#installed`
...
* Added one-word methods like `#snoozed`, `#active` and so-on to the
  `RemoteLock::Alert` class. These replicate the behaviour of similarly
...
* Use `wavefront` rather than `graphite_v2` as the `report` data
  type.
* `RemoteLock::Report` is now a shim around `RemoteLock::Write`, left
  in for backward-compatability. The "correct" way to send data via
  the API is using `RemoteLock::Write`, specifying the `:api` writer.
* `RemoteLock::Credentials` object has new `all` method, which gives
  a hash of proxy *and* API credentials. This is useful for passing
  to `RemoteLock::Write` as it means all writers will work without
  modifying your code.
* Make `RemoteLock::Response#next_item` work with sources. (Or any
  future class which uses a cursor rather than an offset.)
* Make `RemoteLock::User` return a response object containing an
...
* Assume RemoteLock proxies are listening on port 2878, though you
