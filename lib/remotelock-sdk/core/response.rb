...
module RemoteLock
  ...
  # RemoteLock::Exception::UnparseableResponse exception.
  #
  # Most RemoteLock::Response classes present the returned data in
  # two parts, each accessible by dot notation.
  #
  # @!attribute [r] status
  #   @return [RemoteLock::Types::Status]
  ...
  class Response
    ...
    # Create and return a RemoteLock::Response object
    # @param json [String] a raw response body from the RemoteLock API
    # @param status [Integer] HTTP return code from the API
    # @param opts [Hash] options passed through from calling class.
    # @raise [RemoteLock::Exception::UnparseableResponse] if the
    ...
    def initialize(json, status, opts = {})
      ...
      raise RemoteLock::Exception::UnparseableResponse
    end
    ...
    # A printable version of a RemoteLock::Response object
    # @return [String]
    #
    def to_s
      inspect.to_s
    ...
    def setup_vars(opts)
      @opts   = opts
      @logger = RemoteLock::Logger.new(opts)
    end
    ...
    def build_status(raw, status)
      RemoteLock::Type::Status.new(raw, status)
    end
  end
end
